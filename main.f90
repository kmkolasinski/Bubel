...
open(unit=3,file="nt-17-0-1.xyz")
read(3,*) line
read(3,*) line
! ----------------------------------
! Reading unit cell
...
posA = (/0.0,0.0,-0.1/)
posB = (/0.0,0.0,3.0/) ! direction and range
...
a_Emin = -3.0
a_Emax =  3.0
call lead%bands(output_folder//"bands.dat",-M_PI,+M_PI,M_PI/60.0,a_Emin,a_Emax)
...
!! qsystem%qnnbparam%NNB_FILTER = QSYS_NNB_FILTER_CHECK_ALL
...
! call lead%init_lead(lead_shape,lead_translation_vec,qsystem%atoms)
! a_Emin =-0.3 / A0 / 1000.0 ! converting from [meV] to atomic units
! call lead%print_lead(output_folder//"lead.xml",qsystem%atoms)
! a_Emax = 0.1 / A0 / 1000.0 ! converting from [meV] to atomic units
! a_Emax = 0.002
! call lead%bands(output_folder//"bands.dat",-M_PI/3,+M_PI/3,M_PI/60.0,a_Emin,a_Emax)
! ! ----------------------------------------------------------
! ! X. Clean memory...
! ! ----------------------------------------------------------
! call lead%destroy()
...
!        if( abs(xdiff) ==  1 .and. ydiff == 0 ) then
!            connect = .true.
!            coupling_val = xdiff*rs
!        else if( xdiff ==  0 .and. abs(ydiff) == 1 ) then
!            connect = .true.
!            coupling_val = -ydiff*rs*II
!        endif

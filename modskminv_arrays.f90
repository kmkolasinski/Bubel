...
subroutine realloc_fl(new_size,gij,Iij,old_size)
    integer     :: new_size,old_size
    complex(16),allocatable :: gij(:,:)
    integer,allocatable     :: Iij(:)

    if(new_size /= old_size) then
        deallocate(gij,Iij)
        allocate(Gij(new_size,new_size))
        allocate(Iij(new_size))
        old_size = new_size
    endif
end subroutine realloc_fl
! -----------------------------------------------------------
!                           SKMINV
...
subroutine invert(this,elements,values)
    ...
    complex(16),allocatable ::fl_new_Gij(:,:),fl_old_Gij(:,:)
    integer,allocatable     ::fl_new_Iij(:),fl_old_Iij(:),fl_id(:),fl_id_less(:)
    integer :: fl_new_no_sites,fl_old_no_sites
    integer :: no_bonds_less
    complex(16),allocatable  :: Tij(:) , Tji(:)
    ...
    allocate(oGij1(1,no_elems))
    allocate(oGij2(1,no_elems))
    allocate(nGij1(1,no_elems))
    allocate(nGij2(1,no_elems))
...
!    call this%flOld%copy_frontline(this%flNew)
    ...
    allocate(fl_new_Gij(1,1))
    allocate(fl_old_Gij(1,1))
    allocate(fl_new_Iij(1))
    allocate(fl_old_Iij(1))
    allocate(fl_id(1))
    allocate(fl_id_less(1))

    fl_new_Gij = 1/this%nodes(1)%H0
    fl_new_Iij = 1
    fl_old_Gij = fl_new_Gij
    fl_old_Iij = fl_new_Iij

    fl_new_no_sites = 1
    fl_old_no_sites = 1

    fl_id = 1
    allocate(Tij(1),Tji(1))
    ...
    do c_node = 2 , this%no_nodes
        ...
        no_bonds_less = this%nodes(c_node)%no_bonds_less
        deallocate(Tij,Tji,fl_id_less)
        allocate(Tij(no_bonds_less),Tji(no_bonds_less),fl_id_less(no_bonds_less))
        do s1 = 1 , this%nodes(c_node)%no_bonds_less
            Tij(s1) = this%nodes(c_node)%Tij(s1)
            Tji(s1) = this%nodes(c_node)%Tji(s1)
            fl_id_less(s1) = this%nodes(this%nodes(c_node)%Iij_less(s1))%fl_id
        enddo
        ...
        tmpC = 0
        do s1 = 1 , no_bonds_less
            i1 = fl_id_less(s1)
            do s2 = 1 , no_bonds_less
                i2   = fl_id_less(s2)
!                tmpC = tmpC + this%nodes(c_node)%Tij(s1)*this%nodes(c_node)%Tji(s2)*this%flOld%Gij(i2,i1)
                tmpC = tmpC + Tij(s1)*Tji(s2)*fl_old_Gij(i2,i1)
            enddo ! end of s2
        enddo ! end of s1
...
!            if(this%nodes(this%flOld%Iij(p))%no_used < this%nodes(this%flOld%Iij(p))%no_bonds )then
            if(this%nodes(fl_old_Iij(p))%no_used < this%nodes(fl_old_Iij(p))%no_bonds )then
                k = k + 1
            endif
        enddo
...
!        call this%flNew%init_frontline(no_fl) ! allocate memory for new fronline
        call realloc_fl(no_fl,fl_new_Gij,fl_new_Iij,fl_new_no_sites)
...
!            this%flNew%Iij(k)         = c_node
        ...
        if(no_fl /= size(fl_id))then
            deallocate(fl_id)
            allocate(fl_id(no_fl))
        endif
        fl_id = 0
        do p = 1 , fl_new_no_sites
            fl_id(p) = this%nodes(fl_new_Iij(p))%fl_id
        enddo

        if(SKMINV_DEBUG_LEVEL == 1) then
            ! For the last step this should be equal to last element
            print"(A,i,A,10000i)","New fron line size:",no_fl," fl. ids:",fl_new_Iij
        endif
...
!            p   = this%flNew%Iij(k) ! atom do ktorego nalezy nowy gj
            p   = fl_new_Iij(k) ! atom do ktorego nalezy nowy gj
            GXA = 0
            GAX = 0
!            i1  = this%nodes(p)%fl_id
            i1  = fl_id(k)
!            print*,k, i1 , size(fl_id)
            do s1 = 1 , no_bonds_less
!                q  = this%nodes(c_node)%Iij_less(s1)
                i2 = fl_id_less(s1)
!                i2 = this%nodes(q)%fl_id
                GXA = GXA + Tij(s1)*fl_old_Gij(i1,i2)
                GAX = GAX + Tji(s1)*fl_old_Gij(i2,i1)
            enddo
            fl_new_Gij(k,no_fl) = -GXA*GAA
            fl_new_Gij(no_fl,k) = -GAX*GAA
        enddo
...
!            i2 = this%nodes(s2)%fl_id
            i2 = fl_id(p)
            fl_new_Gij(k,p) = fl_old_Gij(i1,i2)  &
                                + fl_new_Gij(k,no_fl)*fl_new_Gij(no_fl,p)/GAA
        enddo
        enddo
        ! Fill last element of the matrix
        fl_new_Gij(no_fl,no_fl) = GAA
        ...
        if(no_fl /= size(nGij1,2))then
            deallocate(nGij1)
            deallocate(nGij2)
            allocate(nGij1(no_fl,c_elem))
            allocate(nGij2(no_fl,c_elem))
        endif
        ...
        if(no_fl /= size(oGij1,2))then
            deallocate(oGij1)
            deallocate(oGij2)
            allocate(oGij1(no_fl,c_elem))
            allocate(oGij2(no_fl,c_elem))
        endif
        oGij1 = nGij1
        oGij2 = nGij2
...
!        do p = 1, this%flOld%no_sites
        do p = 1, fl_old_no_sites
            if(this%nodes(fl_old_Iij(p))%no_used < this%nodes(fl_old_Iij(p))%no_bonds )then
                k = k + 1
                this%nodes(fl_old_Iij(p))%fl_id = k
!                fl_id(p) = k
            endif
        enddo
        if(this%nodes(c_node)%no_used < this%nodes(c_node)%no_bonds) then
...
!            fl_id(no_fl) = k
        ...
        ! copy new to old
!        call this%flOld%copy_frontline(this%flNew)
        call realloc_fl(no_fl,fl_old_Gij,fl_old_Iij,fl_old_no_sites)
        fl_old_Gij = fl_new_Gij
        fl_old_Iij = fl_new_Iij
